...
module.exports.signup_handler = async (req, res) => {
    const { fullName, email, staffId, gender, password, role } = req.body;
    const user = await Student.findOne({ email });
    if (user) {
        return res.status(400).json({ errorMessage: "User Already Exist" });
    }

    const lecturer = new Student({ fullName, email, staffId, gender, password, role });
...
module.exports.rateLecturer = async (req, res) => {
    const { _id } = req.user;
    const { question, star, lecturerId } = req.body
    try {
        const lecturer = await Lecturer.findById(lecturerId);
        const alreadyRated = lecturer.ratings.find((userId) => userId.postedBy.toString() === _id.toString());

        if (alreadyRated) {
            const updateRating = await Lecturer.updateOne({ ratings: { $elemMatch: alreadyRated }}, {  $set: {'ratings.$.star': star } }, { new : true})
            return res.status(200).json(updateRating);
         }

        if (!alreadyRated) {
            const rateLecturer = await Lecturer.findByIdAndUpdate(lecturerId, {
                $push: {
                    ratings: {
                        star: star,
                        postedId: _id,
            ...
            return res.status(200).json({ successMessage: 'Lecturer rating was successful.', rateLecturer });
        }

        const getAllratings = await Lecturer.findById(lecturerId)
        let totalRatings = getAllratings.ratings.length
        let ratingsSum = getAllratings.ratings.map((item) => item.star).reduce((prev, current) => prev + current, 0)
        let actualRating = Math.round(ratingsSum / totalRatings)
        const finalLecturer = await Lecturer.findByIdAndUpdate(lecturerId, {
            totalRatings: actualRating
        }, { new : true})
        console.log(finalLecturer)
    } catch (error) {
        return res.status(500).json({
            errorMessage: "Something went wrong while rating lecturer!. Please try again",
        });
    } finally {
        console.loog('rating success')
    }
}